...
export default function Signup() {
  ...
  const filterAdmissionIds = (inputValue) => {
    if (!inputValue) {
      setFilteredAdmissionIds([]);
    } else {
      const filtered = admissionIds.filter((id) =>
        id.toLowerCase().includes(inputValue.toLowerCase())
      );
      setFilteredAdmissionIds(filtered);
    }
  };
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5225/api/User/GetAllUsers")
      .then((response) => {
    ...
    if (userDetails.Role === "student") {
      apiUrl = "http://localhost:5225/api/Student/GetAllStudents";
    } else if (userDetails.Role === "teacher") {
      apiUrl = "http://localhost:5225/api/Teacher/GetAllTeacher";
    }
    if (apiUrl) {
      axios
        .get(apiUrl)
        ...
        .catch((error) => {
          console.error("Failed to fetch Admission IDs:", error);
        });
    }
  }, [userDetails.Role]);
...
const validateAllFields = () => {
  const errors = {
    EmailId: validateInput(userDetails.EmailId, "Email"),
    PhoneNum: validateInput(userDetails.PhoneNum, "Phone Number"),
    AdmissionID: validateInput(userDetails.AdmissionID, "Admission ID"),
    Role: validateInput(userDetails.Role, "Role"),
    UserName: validateInput(userDetails.UserName, "Username"),
    Password: validateInput(userDetails.Password, "Password"),
    UserId: validateInput(userDetails.UserId, "UserId"),
  };
  return errors;
};
...
const handleChange = (e) => {
  ...
  if (e.target.id === "AdmissionID") {
    filterAdmissionIds(e.target.value);
  }
  if (e.target.id === "confirmPassword") {
    setConfirmPassword(e.target.value);
  }
  if (e.target.id === "UserId" || e.target.id === "UserName") {
    if (!isUnique(e.target.id, e.target.value)) {
      setErr(`${e.target.id} is already taken.`);
    } else {
      setErr("");
    }
  }
                  ...
                  <form onSubmit={handleSignup}>
                    <div className="row ">
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="mb-3">
                          <div>
                            ...
                            {filteredAdmissionIds.length > 0 && (
                              <ul className="list-group">
                                {filteredAdmissionIds.map((id, index) => (
                                  <li
                                    key={index}
                      ...
                      <div className="d-grid mt-2">
                        ...
                        {err && (
                          <small id="ButtonHelp" className="text-danger">
                            {err}
                          </small>
                        )}
                      </div>
